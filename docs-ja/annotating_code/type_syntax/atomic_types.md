...
原子型は、Psalmで使用されるすべての型情報の基本的な構成要素です。複数の原子型は、[ユニオン型](union_types.md)や[インターセクション型](intersection_types.md)と組み合わせることができます。Psalmでは、docblock構文で多くの異なる種類の原子型を表現できます。

**注意**: 以下のリストの各型をクリックすると、すべての原子型の詳細なドキュメントと使用例を表示できます。
